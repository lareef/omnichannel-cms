...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # If no data found, fallback to the first available (or None)
        if not all_channels:
        ...
        if not all_priorities:
            ...
            return
        if not all_statuses:
            self.stderr.write("No statuses found. Please create at least one status.")
            return

        # Helper to get a random item from a list, or first if random not requested
    ...
    def create_tickets(self, count, all_channels, all_categories, all_departments, all_priorities, all_statuses, all_agents,
        ...
        for i in range(count):
            ...
            # Randomly select channel, category, department, priority, status
            ...
            description = f"This is a test ticket generated by management command. {subject} details: {random.choice(['urgent', 'minor', 'escalated', 'routine'])}."
            ...
            if breach_response:
                ...
                # Set updated_at to 2 hours ago (no activity)
